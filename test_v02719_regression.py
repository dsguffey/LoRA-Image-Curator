...
def test_current_version_is_consistent() -> None:
    """Keep public and packaging identity synchronized at v0.27.19."""
    assert APP_VERSION == "0.27.19"
    assert "Version 0.27.19" in _source("VERSION.txt")
    assert 'version = "0.27.19"' in _source("pyproject.toml")
    assert "v0.27.19" in _source("README.md")
    assert "v0.27.19" in _source("README.txt")
...
def test_checklist_launcher_is_project_relative_and_first_time_safe() -> None:
    ...
    body = _source("Install Body and File Action Dependencies.bat")
...
def test_github_documentation_matches_dependency_tiers() -> None:
    ...
    for phrase in (
        ...
        "Face analysis, body/pose analysis, native Recycle Bin support, and FFmpeg",
...
def test_current_release_chains_include_v02719() -> None:
    ...
    for member in (
        ...
        '"Setup and Launch LoRA Image Curator.bat"',
        '"Install Base Dependencies.bat"',
        '"test_v02719_regression.py"',
        '"test_v02719_gui.py"',
    ):
        assert member in build
    assert '"test_v02719_regression.py"' in regressions
