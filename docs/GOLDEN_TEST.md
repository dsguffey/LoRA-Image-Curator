...
`test_golden_build.py` is the authoritative release and workspace-handoff
command for LoRA Image Curator v0.27.17.
...
- every maintained non-GUI regression from Milestone 6B through v0.27.17 passes;
...
- the current cumulative Windows/Tk GUI chain through v0.27.17 passes without
...
```text
GOLDEN BUILD PASSED — LoRA Image Curator v0.27.17
```
...
The user has separately confirmed on Windows that v0.27.5's packaged smoke test
passed and deletion from enlarged review reached the Recycle Bin workflow.
