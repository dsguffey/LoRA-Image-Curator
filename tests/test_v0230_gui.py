"""Windows GUI smoke test for v0.23.0 / Milestone 10 Phase 1C.

Run after installing the normal application dependencies:

    python -X dev -m tests.test_v0230_gui
...
Ctrl+A handling, chronological Undo/Redo across selection and catalog actions,
and the curated appearance-theme picker.
"""
...
def run() -> None:
    with tempfile.TemporaryDirectory(prefix="dataset_tools_v0230_gui_") as temporary:
        ...
        try:
            ...
            font_cache = getattr(root, "_dataset_tools_font_cache", {})
            assert font_cache
            assert all(
                isinstance(cached_font, tkfont.Font)
                for cached_font in font_cache.values()
            )
            ...
            # Exercise the same owned teardown path as a real window close.
            # Direct root destruction leaves app-level ``after()`` callbacks
            # registered and makes Tk print invalid-command warnings.
    ...
    print(
        "v0.23.0 GUI smoke test passed: browser-only menus, attached curation "
        "marker, configurable bounded pages, focus-aware selection shortcuts, "
        "chronological selection/catalog Undo/Redo, and appearance themes."
    )
