...
def test_current_version_retains_v02719_setup_history() -> None:
    ...
    assert "Version 0.28.0" in _source("VERSION.txt")
    assert 'version = "0.28.0"' in _source("pyproject.toml")
    assert "v0.28.0" in _source("README.md")
    assert "v0.28.0" in _source("README.txt")
...
def test_current_release_chains_include_v02719() -> None:
    ...
    assert "python -X dev -m tests.test_v0280_regression" in workflow
